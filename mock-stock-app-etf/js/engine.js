// ─── Market Simulation Engine (GBM-based) with Persistence for ETF App ──────
const PRICE_STORAGE_KEY = 'engine_state_etf';
...
export class MarketEngine {
  ...
  _initAsset(asset) {
    ...
    this.prices[asset.ticker] = {
      ...
      prevClose: roundPrice(price * (1 - (Math.random() - 0.5) * 0.008)),
      open: roundPrice(price * (1 + (Math.random() - 0.5) * 0.003)),
      high: roundPrice(price * (1 + Math.random() * 0.005)),
      low: roundPrice(price * (1 - Math.random() * 0.005)),
      change: 0,
      changePercent: 0,
  ...
  _generateHistory(asset, currentPrice) {
    ...
    for (let i = 59; i >= 0; i--) {
      const open = price;
      const dailyReturn = (Math.random() - 0.49) * asset.volatility * 1.2;
      const close = Math.max(0.01, open * (1 + dailyReturn));
      const spread = asset.volatility * 0.3;
  ...
  _generateIntraday(asset, currentPrice) {
    ...
    for (let i = 119; i >= 0; i--) {
      const io = ip;
      const ir = (Math.random() - 0.5) * asset.volatility * 0.15;
      const ic = Math.max(0.01, io * (1 + ir));
      intraday.push({
        time: now - i * 60000,
        open: roundPrice(io), high: roundPrice(Math.max(io, ic)),
        low: roundPrice(Math.min(io, ic)), close: roundPrice(ic),
        volume: Math.floor(Math.random() * 30000) + 2000,
      });
      ip = ic;
    }
  ...
  tick() {
    ...
    for (const asset of this.assets) {
      if (!sectorSentiment[asset.sector]) {
        sectorSentiment[asset.sector] = (Math.random() - 0.5) * 0.001; // smaller fluctuations for ETFs
      }
      const pd = this.prices[asset.ticker];
      if (!pd) continue;
      const prev = pd.price;
      ...
      if (Math.random() < 0.0002) {
        newPrice *= (1 + (Math.random() - 0.45) * 0.02);
      }
      ...
      pd.volume += Math.floor(Math.random() * 300);
