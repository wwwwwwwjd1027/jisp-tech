...
import { getAllAssets, getAssetByTicker, getStockSectors } from './data.js';
...
const DOM = {
  ...
  marketTabStocks: document.getElementById('market-tab-stocks'),
  marketTabCrypto: document.getElementById('market-tab-crypto'),
...
function init() {
  ...
  if (DOM.jispSubmit) {
    DOM.jispSubmit.addEventListener('click', () => {
      ...
      const API_BASE = window.JISP_API_BASE || (window.location.hostname === 'localhost' ? '' : '');
...
function setupListeners() {
  ...
  const setMarketFilter = (filter) => {
    ...
    DOM.marketTabStocks.classList.toggle('active', filter === 'stocks');
    DOM.marketTabCrypto.classList.toggle('active', filter === 'crypto');
  ...
  DOM.marketTabStocks.addEventListener('click', () => setMarketFilter('stocks'));
  DOM.marketTabCrypto.addEventListener('click', () => setMarketFilter('crypto'));
...
// 2. MARKET TAB
let currentMarketTab = 'all'; // 'all', 'stocks', 'crypto', 'watchlist'
...
function renderMarket() {
  ...
  if (currentMarketTab === 'stocks') {
    list = list.filter(a => a.sector !== 'Crypto');
  } else if (currentMarketTab === 'crypto') {
    list = list.filter(a => a.sector === 'Crypto');
  ...
  // Sector filter
  if (currentSectorFilter && currentMarketTab !== 'crypto') {
...
function renderSectorFilters() {
  if (DOM.sectorFilters.children.length > 0 && currentMarketTab === 'crypto') {
...
// Hook tab filter buttons
DOM.marketTabAll.addEventListener('click', () => { currentMarketTab = 'all'; currentSectorFilter = null; renderMarket(); });
DOM.marketTabStocks.addEventListener('click', () => { currentMarketTab = 'stocks'; currentSectorFilter = null; renderMarket(); });
DOM.marketTabCrypto.addEventListener('click', () => { currentMarketTab = 'crypto'; currentSectorFilter = null; renderMarket(); });
DOM.marketTabWatchlist.addEventListener('click', () => { currentMarketTab = 'watchlist'; currentSectorFilter = null; renderMarket(); });
